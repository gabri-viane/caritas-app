...
export function showFams(success_handler, error_handler, k_url) {
    fam({ ...{ method: 'get', k: "get/" + k_url } }, (dt) => { success_handler(dt) }, (dt) => { error_handler(dt) });
}
...
export function deleteFam(success_handler, error_handler, k_url) {
    fam({ ...{ method: 'delete', k: "delete/" + k_url.idfam } }, (dt) => { success_handler(dt) }, (dt) => { error_handler(dt) });
}

export function createComp(success_handler, error_handler, k_url, values) {
    fam({ ...values, ...{ method: 'post', k: "add/comp/" + k_url } }, (dt) => { success_handler(dt) }, (dt) => { error_handler(dt) });
}

export function updateComp(success_handler, error_handler, k_url, values) {
    fam({ ...values, ...{ method: 'put', k: "update/" + k_url.idfam + "/comp/" + k_url.idcomp } }, (dt) => { success_handler(dt) }, (dt) => { error_handler(dt) });
}

export function deleteComp(success_handler, error_handler, k_url) {
    fam({ ...{ method: 'delete', k: "delete/" + k_url.idfam + '/comp/' + k_url.idcomp } }, (dt) => { success_handler(dt) }, (dt) => { error_handler(dt) });
}

export function requestParentele(success_handler) {
    extra({ ...{ method: 'get', k: "get/parentele" } }, success_handler, (dt) => { alert(dt) });
}

export function requestComponent(success_handler, error_handler, k_url) {
    fam({ ...{ method: 'get', k: "get/idfam/" + k_url.idfam + "/comp/" + k_url.idcomp } }, success_handler, (dt) => { error_handler(dt) });
}
